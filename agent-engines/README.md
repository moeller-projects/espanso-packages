...
|---|---|---|
| `;ado-read-wi` | ado-gateway | Fetch a work item and emit the handoff JSON |
| `;ado-read-pr` | ado-gateway | Parse a PR URL and return the pr-comments handoff |
| `;ado-read-wi-pr` | ado-gateway | Fetch work item + linked PR comments, emit handoff envelope |
...
| `;tests-plan` | test-engine | Design a test plan |
| `;tests-e2e` | test-engine | Add Playwright E2E tests |
| `;ops-ci` | ops-engine | Review a GitHub Actions workflow |
| `;ops-threat` | ops-engine | Threat-model a deployment path |
| `;doc-readme` | doc-engine | Write a README |
...
| `;caveman-full` | caveman | Full-mode triage |
...
|---|---|---|
| `;pipe-ticket` | Full ticket → ship loop | Fetch WI → onboard → spec → slice → test plan → WI comment |
...
In v0.1.0, placeholders like `<org>/<project>` or `<wi-id>` are inline text — replace them manually before sending. A future version will use Espanso form variables so the editor pops up a dialog to fill each field before expansion. Example of what this will look like for `;ado-read-wi`:

```yaml
- trigger: ";ado-read-wi"
  word: true
  label: "ado-gateway: fetch work item, emit handoff"
  replace: "Fetch ADO work item {{wi_id}} from {{org}}/{{project}} and emit the handoff JSON. Do not draft a spec."
  vars:
    - name: wi_id
      type: form
      params:
        layout: "Work item ID: {{wi_id}}"
```
